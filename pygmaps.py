...
class pygmaps:
	...
	def __init__(self, centerLat, centerLng, zoom ):
		...
		self.coloricon = 'http://maps.google.com/mapfiles/kml/pal2/icon33.png'
			# 'http://chart.apis.google.com/chart?cht=mm&chs=22x24&chco=FFFFFF,AAXXAA,000000&ext=.png'

	def setgrids(self,slat,elat,latin,slng,elng,lngin):
		self.gridsetting = [slat,elat,latin,slng,elng,lngin]

	def addpoint(self, lat, lng, coloricon = 'http://chart.apis.google.com/chart?cht=mm&chs=22x24&chco=FFFFFF,AAXXAA,000000&ext=.png', color = '#FF0000'):
		self.coloricon = coloricon
		self.points.append((lat,lng,coloricon,color[1:]))
	...
	def drawpoint(self,f,lat,lon,icon,color):
		f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n'%(lat,lon))
		f.write('\t\tvar img = new google.maps.MarkerImage(\'%s\');\n' % (icon.replace('XXXXXX',color)))
